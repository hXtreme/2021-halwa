...
pathTest :: String -> Test
pathTest rawAst = 
  "ProgramParserTest1" ~: (Right pathProgram) ~?= ast
    where
...
evenProgram = DL.Program {DL.declarations=[d1, d2, d3], DL.rules=[rule2], DL.facts=[fact1, fact2, fact3, fact4, fact5, fact6, fact7, fact8, fact9, fact10, fact11, fact12, fact13], DL.disjunction=[], DL.queries=[query1]}        
      where
        ...
        var2 = Datalog.Variable.Variable {name="O"}
        ...
        a19 = Atom {Datalog.Atom.predicate="even", args=[Constant const3]}
...
evenTest :: String -> Test
evenTest rawAst = 
  "ProgramParserTest2" ~: (Right evenProgram) ~?= ast
    where
      parsedItem (Left e) = Left e
      parsedItem (Right (L _ it)) = Right it
      ast = parsedItem (P.parse parser rawAst)
...
tParserTestsFromFile s1 s2 =
  "tProgramParserTests"
    ~: TestList [pathTest s1, evenTest s2]
...
test_all = do
  ...
  runTestTT $
    TestList
      [ tProgramParserTests,
        tParserTestsFromFile rawAstPath rawAstEven
      ]
